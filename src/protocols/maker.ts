...
export async function monitorMaker(env: any, provider: any, fromBlockHex: string, toBlockHex: string, now: number, alerts: any[], activityTracker: Map<string, number>, updateActor: any, getTransaction: any, getThreatPrefix: any, formatActorDescription: any) {
  ...
  for (const log of makerLogs) {
    ...
    const baseAlert = {
      ...
      chain: 'ethereum', protocol: 'maker', txHash: log.transactionHash, blockNumber, timestamp: now,
    };

    if (decoded.name === 'Rely' || decoded.name === 'Deny') {
      if (tx?.from?.toLowerCase() !== MAKER_GOV.toLowerCase()) {
        const actorRecord = await updateActor(actor, 100, `Maker Access Control: ${decoded.name}`, env, log.transactionHash, blockNumber);
        alerts.push({ 
          ...baseAlert, 
          id: `${log.transactionHash}-maker-acl`, 
          severity: 'critical', 
          type: 'MAKER_ACL_CHANGE',
          title: 'Maker PSM Access Control Change', 
          description: formatActorDescription(`Authorization change detected: ${decoded.name} for user ${decoded.args.usr}`, actorRecord), 
      ...
      if (decoded.args.value > 5000000n * 10n**6n) {
        const actorRecord = await updateActor(actor, 15, `Maker PSM ${decoded.name}`, env, log.transactionHash, blockNumber);
        alerts.push({ 
          ...baseAlert, 
          id: `${log.transactionHash}-maker-swap`, 
          severity: 'high', 
          type: 'MAKER_PSM_LARGE_MOVEMENT',
          title: `Maker PSM Large ${decoded.name === 'SellGem' ? 'Inflow' : 'Outflow'}`, 
          description: formatActorDescription(`Large movement in Maker PSM: ${(Number(decoded.args.value) / 1e6).toFixed(2)} USDC`, actorRecord), 
          actorScore: actorRecord.score, actorHistoryCount: actorRecord.eventCount, recentEvents: actorRecord.recentEvents, threatPrefix: getThreatPrefix(actorRecord) 
        });
      }
    }
